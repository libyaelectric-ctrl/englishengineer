...
async function run() {
  console.log(`Tracing Stripe Card Fields... Email: ${email}`);
  ...
  try {
    ...
    // Click "Kart" accordion button natively via page.evaluate()
    console.log(
      'Clicking "Kart" accordion button natively via page.evaluate()...'
    );
    await page.evaluate(() => {
    ...
    await page.waitForTimeout(5000); // Wait for card fields to render

    await page.screenshot({
      path: join(ARTIFACT_DIR, 'stripe_card_fields.png'),
    });
    console.log('Screenshot of expanded card fields captured.');

    // Print all inputs
    console.log(
      'Listing all inputs on the Stripe Checkout page after clicking Kart:'
    );
    ...
    console.error('Error during Stripe card trace:', err);
  } finally {
    await browser.close();
  }
}
