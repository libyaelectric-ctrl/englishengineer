...
const getFeatureLabels = (landingCopy: ReturnType<typeof getLandingTranslations>): Record<string, string | undefined> => ({
...
const getCardClasses = (compact: boolean, popular: boolean): string => {
  const base = 'relative flex h-full flex-col justify-between rounded-[1.25rem] p-3 bg-surface transition-all duration-300 shadow-sm';
  const desktop = 'relative flex h-full min-h-[520px] flex-col justify-between rounded-[var(--radius-card)] p-5 bg-surface transition-all duration-300 shadow-sm';
  const border = popular ? 'border-2 border-primary shadow-lg' : 'border border-border-soft';
  return compact ? `${base} ${border}` : `${desktop} ${popular ? 'border-2 border-primary shadow-xl' : 'border border-border-soft'}`;
};
  ...
  return (
    <div className="mb-2">
      <div className="flex flex-wrap items-baseline gap-1.5">
        ...
        <span className={compact ? 'font-mono text-2xl font-extrabold tracking-tight text-foreground' : 'font-mono text-3xl font-extrabold tracking-tight text-foreground'}>
    ...
    <article className={cardClasses}>
      ...
      <div className="relative z-10 flex h-full flex-col justify-between">
        <div className="min-h-0">
          ...
          <PriceDisplay tier={tier} price={price} currency={currency} isAnnual={isAnnual} isTeam={isTeam} compact={compact} copy={copy} />
