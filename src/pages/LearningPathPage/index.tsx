...
export const LearningPathPage: React.FC = () => {
  ...
  const handleStartLevel = (levelId: string, isLocked: boolean) => {
    if (isLocked) return;
    if (hearts <= 0) {
      alert('Canınız tükendi! Lütfen canlarınızı yenileyin veya bekleyin.');
      return;
    }
      ...
      <header className="sticky top-0 z-30 bg-[var(--surface)]/90 backdrop-blur-md border-b border-[var(--color-border-soft)] shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 sm:gap-6">
            ...
            {/* Gems */}
            <div className="flex items-center gap-1.5 bg-sky-500/10 px-3 py-1.5 rounded-full border border-sky-500/20" title="Elmaslar">
            ...
            <div key={unit.id} className="space-y-6">
              ...
              <div className="flex flex-col items-center gap-6 py-4 relative">
                {unit.levels.map((level, lIdx) => {
                  ...
                  // Level is unlocked if it's the first level, or previous level is completed
                  const isUnlocked =
                    lIdx === 0 && uIdx === 0
                      ? true
                      : lIdx === 0
                  ...
                  // Create S-curve winding offset for Duolingo feel
