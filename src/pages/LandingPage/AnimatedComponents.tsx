...
export function AnimatedCard({
  ...
  const cardStyle: React.CSSProperties = {
    ...
    color: dark ? '#fff' : 'var(--color-foreground)',
    opacity: isVisible ? 1 : 0,
    transform: isVisible ? 'translateY(0)' : 'translateY(28px)',
    transition: `opacity 720ms cubic-bezier(0.16,1,0.3,1) ${delay}ms, transform 720ms cubic-bezier(0.16,1,0.3,1) ${delay}ms`,
  };
      ...
      <div
        ...
        style={{
          background: dark
            ...
            : 'radial-gradient(600px circle at var(--mouse-x) var(--mouse-y), rgba(0,71,187,0.08), transparent 40%)',
        }}
      />
      {children}
    </div>
...
export function SectionIntro({
  ...
  return (
    <div className={`mb-10 ${align === 'center' ? 'text-center' : ''}`}>
      <span className="inline-block rounded bg-soft border border-border-soft px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-primary mb-4">
        {eyebrow}
      </span>
