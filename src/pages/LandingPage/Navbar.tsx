...
export function Navbar() {
  ...
  const onListboxKeyDown = (event: React.KeyboardEvent<HTMLUListElement>) => {
    ...
    langListRef.current
      ?.querySelector<HTMLElement>(`#${optionId(options[nextIndex].id)}`)
      ?.scrollIntoView({ block: 'nearest' });
  };
  return (
    <header
      className="fixed left-0 right-0 top-0 z-50 border-b border-border-soft bg-background/88 text-foreground shadow-card backdrop-blur-xl"
      style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}
    >
      <div className="mx-auto max-w-7xl px-3 sm:px-6">
        <div className="flex h-14 items-center gap-2">
          <Link to="/" className="group flex shrink-0 items-center gap-2">
            <div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-[var(--radius-button)] ring-1 ring-border-soft transition-transform duration-200 group-hover:scale-105">
              <img
                src="/brand/logo.svg"
                alt="EngVox"
          ...
          <div className="relative" ref={langRef}>
            <button
              ...
              className="flex h-10 items-center gap-1 rounded-[var(--radius-button)] border border-border-soft bg-surface px-2 text-sm transition-colors hover:bg-surface-hover"
            ...
            >
              ...
              <span className="hidden text-xs font-black text-muted-copy sm:inline">
            ...
            {langOpen && (
              <div className="absolute left-0 mt-2 w-48 origin-top-left rounded-[var(--radius-card)] border border-border-soft bg-surface/95 p-1 shadow-pop backdrop-blur-xl animate-in fade-in-0 zoom-in-95">
                <ul
                  ref={langListRef}
                  id={listboxId}
                  role="listbox"
                ...
                >
                  {INTERFACE_LANGUAGES.map((lang) => (
                    <li
                      ...
                      className={`flex cursor-pointer items-center gap-2 rounded-[var(--radius-button)] px-3 py-2 text-sm transition-colors ${activeLangId === lang.id ? 'bg-primary/10 font-black text-primary' : 'text-muted-copy hover:bg-surface-hover hover:text-foreground'}`}
                    >
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            <button
              ...
              className="hidden h-10 items-center rounded-[var(--radius-button)] border border-primary/30 bg-primary/10 px-3 text-[11px] font-black text-primary transition-colors hover:bg-primary/15 sm:inline-flex"
            >
