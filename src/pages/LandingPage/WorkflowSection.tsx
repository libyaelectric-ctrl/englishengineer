...
export function WorkflowSection() {
  return (
    <section className="border-t border-[#d9d9e3] bg-[#faf8ff] bg-[linear-gradient(to_right,#8080800b_1px,transparent_1px),linear-gradient(to_bottom,#8080800b_1px,transparent_1px)] bg-[size:24px_24px] px-6 py-12 md:px-12 md:py-20">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {WORKFLOW.map((item, index) => (
            <AnimatedCard key={item.title} delay={index * 60} className="p-4">
              <div className="relative z-10 overflow-hidden rounded-[4px] border border-[#d9d9e3] bg-[#111]">
                <img
                  src={item.image}
                  alt=""
