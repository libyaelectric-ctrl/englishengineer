...
export const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] pb-14 text-[var(--foreground)] transition-colors duration-300">
      ...
      <section ref={heroRef} className="relative overflow-hidden py-20 md:py-28">
        ...
        >
          <motion.div
            ...
            className="mb-6 inline-flex items-center gap-2 rounded-[var(--radius-card)] border border-[var(--color-primary)]/20 bg-[var(--color-primary)]/5 px-4 py-1.5 text-xs font-semibold text-[var(--color-primary)]"
          >
            <motion.span
              variants={iconHover}
          ...
          <motion.p
            variants={staggerItem}
            className="mx-auto mt-6 max-w-2xl text-lg text-[var(--color-muted-copy)]"
          >
            {t.heroSubtitle}
          </motion.p>
          ...
          <motion.div
            variants={staggerItem}
            className="mt-6 flex flex-wrap items-center justify-center gap-4 text-xs text-[var(--color-muted-copy)]"
          >
      ...
      <section
        ref={socialProofRef}
        className="border-y border-[var(--color-border-soft)] bg-[var(--color-surface)] py-8"
      >
        <motion.div
          variants={staggerContainer}
          initial="hidden"
      ...
      >
        ...
        >
          ...
          >
            {ENGINEERING_DISCIPLINES.map((id, _i) => (
              <motion.div
                key={id}
                variants={staggerItem}
          ...
          <motion.div
            ...
            initial="hidden"
            animate={disciplinesInView ? 'visible' : 'hidden'}
            className="mt-8 grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-6"
          >
            {FEATURES.map(({ icon: Icon, key, color }, _i) => (
              <motion.div
                key={key}
                variants={staggerItem}
      ...
      <section ref={workflowRef} className="py-20">
        ...
        >
          <motion.h2 variants={staggerItem} className="text-center text-3xl font-bold">
            {t.workflowTitle}
          </motion.h2>
          <motion.div
            variants={staggerContainer}
