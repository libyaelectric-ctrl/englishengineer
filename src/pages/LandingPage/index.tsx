            ...
            <motion.section key="hero" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.55, ease: [0.16, 1, 0.3, 1] }} className="absolute inset-0 grid h-full place-items-center overflow-hidden px-4 pb-16 pt-14 sm:pb-16 sm:pt-14">
              <div className="mx-auto grid w-full max-w-7xl items-center gap-4 lg:grid-cols-[1fr_0.86fr] xl:gap-6">
                <div className="text-center lg:text-left">
                  ...
                  <h1 className="mx-auto mt-3 max-w-5xl text-[clamp(2.1rem,6.1vw,5.25rem)] font-black leading-[0.92] tracking-[-0.065em] text-slate-950 lg:mx-0 dark:text-white"><span className="block">{t.heroTitle1}</span><span className="block bg-gradient-to-r from-cyan-600 via-slate-800 to-indigo-600 bg-clip-text py-1 text-transparent dark:from-cyan-200 dark:via-white dark:to-fuchsia-200">{t.heroTitleHighlight}</span><span className="block text-slate-800 dark:text-white/72">{t.heroTitle2}</span></h1>
                ...
                <div className="relative mx-auto hidden w-full max-w-lg lg:block"><div className="relative overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white/95 p-4 shadow-xl backdrop-blur-2xl dark:border-white/12 dark:bg-[#0c1425]/85"><div className="mb-3 flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-cyan-700 dark:text-cyan-200">{t.cockpitLabel}</p><p className="text-sm font-bold text-slate-700 dark:text-white/70">{t.cockpitTitle}</p></div><span className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-200">{t.cockpitStatus}</span></div><div className="grid gap-2.5 sm:grid-cols-3">{metrics.map(({ label, value, icon: Icon }) => (<div key={label} className="rounded-2xl border border-slate-200 bg-white p-3 dark:border-white/10 dark:bg-black/20"><Icon className="mb-2 h-5 w-5 text-cyan-700 dark:text-cyan-200" /><p className="text-xl font-black tracking-tight">{value}</p><p className="text-[11px] font-bold text-slate-600 dark:text-slate-300">{label}</p></div>))}</div><div className="mt-3 rounded-3xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-white/[0.06]"><div className="mb-3 flex items-center justify-between"><span className="text-sm font-black">{t.skillMap}</span><Zap className="h-5 w-5 text-amber-500 dark:text-amber-200" /></div><div className="space-y-2.5">{FEATURES.slice(0, 4).map(({ key, accent }, index) => (<div key={key} className="grid grid-cols-[5.4rem_1fr_2.3rem] items-center gap-3 text-[11px] font-bold text-slate-700 dark:text-slate-200"><span>{translate(`nav.${key}`)}</span><span className="h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-white/10"><motion.span className={`block h-full rounded-full bg-gradient-to-r ${accent}`} initial={{ width: 0 }} animate={{ width: `${78 + index * 5}%` }} transition={{ duration: 1.1, delay: 0.25 + index * 0.12 }} /></span><span className="text-right text-cyan-700 dark:text-cyan-100">{78 + index * 5}%</span></div>))}</div></div></div></div>
              </div>
            </motion.section>
          )}
