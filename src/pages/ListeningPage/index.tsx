  ...
  return (
    <PageContainer className="space-y-6">
      <PageHeader
        ...
        actions={
          <span className="rounded-[var(--radius-button)] border border-border-soft bg-surface px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-muted-copy">
      ...
      <div className="space-y-6 pt-2">
        ...
        {!workspaceOpen ? (
          <SectionCard
            ...
            subtitle="Choose a level-safe task"
            icon={Headphones}
          >
            <div className="mb-4 flex flex-wrap gap-2">
              {CATEGORIES.map((cat) => (
                <button
                  ...
                  className={`min-h-11 rounded-[var(--radius-button)] border px-3.5 text-xs font-bold transition-all ${categoryFilter === cat ? 'border-primary bg-primary text-primary-foreground shadow-sm' : 'border-border-soft bg-surface text-muted-copy hover:bg-primary/5 hover:text-primary'}`}
                >
                  {cat}
                </button>
              ))}
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {filteredMissions.map((mission) => (
                <article
                  key={mission.id}
                  className="group rounded-[var(--radius-card)] border border-border-soft bg-surface p-5 transition-all duration-200 hover:border-primary/30 hover:shadow-card"
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="rounded-[var(--radius-button)] border border-border-soft bg-surface px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-primary">
                        {mission.cefrLevel}
                      </span>
                      <LevelAccessBadge
                  ...
                  <h2 className="mt-3 font-bold tracking-tight text-foreground transition-colors group-hover:text-primary">
                    {mission.title}
                  </h2>
                  <p className="mt-2 text-sm font-normal leading-6 text-muted-copy">
                    {mission.description}
                  </p>
                  <Button
