...
export const WritingFeedbackPanel = ({
  ...
  <div className="space-y-4">
    ...
    <div className="grid grid-cols-2 gap-3">
      {scores.map((score) => (
        <div key={score.label} className="rounded-[4px] border-2 border-[#0047bb] bg-surface p-3">
          <p className="text-[10px] font-bold uppercase text-muted-copy">{score.label}</p>
    ...
    {feedback && (
      <div className="rounded-[4px] border-2 border-[#0047bb] bg-surface p-4">
        <p className="text-[10px] font-bold uppercase text-muted-copy mb-2">Feedback</p>
        <p className="text-xs leading-relaxed text-foreground whitespace-pre-wrap">{feedback}</p>
      </div>
    )}

    {corrections.length > 0 && (
      <div className="rounded-[4px] border border-amber-300 bg-amber-50 p-4">
        <p className="text-[10px] font-bold uppercase text-amber-700 mb-2">Corrections</p>
