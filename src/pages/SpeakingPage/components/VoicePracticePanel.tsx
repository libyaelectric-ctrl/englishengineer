...
export const VoicePracticePanel = ({
  ...
  return (
    <div className="space-y-4 font-sans">
      {/* Active Voice Workspace - styled like Vocal Lab/Studio Equipment */}
      <div className="rounded-[4px] border border-[#d9d9e3] bg-[#f3f3fd] p-5 flex flex-col items-center justify-center min-h-32 relative overflow-hidden shadow-sm">
        {/* Subtle grid layer for technical look */}
            ...
            <div className="grid gap-2 grid-cols-2 sm:grid-cols-4 lg:grid-cols-6">
              {phonemeFeedback.map((item, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="rounded-[4px] bg-white border border-[#d9d9e3] p-3 text-center shadow-sm flex flex-col justify-between"
                  >
                    <div>
                      <p className="text-xs font-bold text-foreground">
                        {item.word}
                    ...
                    <div className="mt-3 flex items-center justify-center gap-1">
                      ...
                      <span
                        className={`text-[9px] font-bold ${
                          isAccurate ? 'text-[#0047bb]' : 'text-amber-600'
                        }`}
                      >
                        {item.score}%
                      </span>
