...
const PlacementPage = () => {
  ...
  if (result)
    return (
      <div className="mx-auto max-w-3xl py-4 sm:py-8">
        <section className="rounded-[var(--radius-card)] border border-border-soft bg-surface p-6 shadow-card sm:p-8">
          <CheckCircle2 className="h-8 w-8 text-success" />
          <p className="mt-5 text-xs font-bold uppercase text-success">Placement complete</p>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {[
              ['Score', `${result.score}%`],
              ['Priority', result.priorityAreas[0] ?? 'Consolidation'],
              ['Evidence', 'Local rules'],
            ].map(([label, value]) => (
              <div
                key={label}
  ...
  return (
    <div className="mx-auto max-w-3xl py-4 sm:py-8">
      <section className="overflow-hidden rounded-[var(--radius-card)] border border-border-soft bg-surface shadow-card">
        <header className="border-b border-border-soft bg-surface-hover p-5 sm:p-7">
          <div className="flex items-center gap-3">
            <ClipboardCheck className="h-5 w-5 text-primary" />
            <div>
              <p className="text-xs font-bold uppercase text-primary">Placement MVP</p>
          ...
          <ProgressBar
            ...
            className="mt-5"
          />
          <p className="mt-2 text-xs font-semibold text-muted-copy">
            Question {currentIndex + 1} of {PLACEMENT_QUESTIONS.length}
          </p>
        </header>
        <div className="p-5 font-sans sm:p-7">
          <span className="rounded-[var(--radius-button)] border border-border-soft bg-background px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-muted-copy">
          ...
          <div className="mt-5 grid gap-3">
            {question.choices.map((choice, choiceIndex) => (
              <button
                ...
                className={`min-h-12 rounded-[var(--radius-button)] border px-4 py-3 text-left text-xs font-bold uppercase tracking-wider shadow-sm transition-all ${answers[question.id] === choiceIndex ? 'border-primary/40 bg-primary/10 text-foreground' : 'border-border-soft bg-surface text-muted-copy hover:border-primary'}`}
              >
                {choice}
              </button>
            ))}
        ...
        <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-border-soft bg-background p-4 sm:px-7">
          <Button
            ...
            className="border border-border-soft bg-surface text-primary hover:bg-surface-hover"
          >
