...
export const FreePlanButton = ({ currentUser }: { currentUser: { id: string } | null }) => (
  <Link
    ...
    className="mt-4 flex h-9 w-full items-center justify-center rounded-[var(--radius-card)] border border-border-soft bg-surface text-xs font-bold uppercase tracking-wider hover:bg-surface-hover transition-all cursor-pointer shadow-sm text-foreground"
  >
...
export const PlanAction = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`flex h-9 w-full items-center justify-center rounded-[var(--radius-card)] text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shadow-sm ${style}`}
    >
      {label}
    </button>
  );
