  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-20 pt-20 text-slate-950 dark:bg-[#040611] dark:text-white">
      ...
      <article className="relative mx-auto max-w-4xl px-4 sm:px-6 md:px-8">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-slate-900/10 bg-white/70 p-3 shadow-sm backdrop-blur-2xl dark:border-white/10 dark:bg-white/[0.06]">
          <div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-cyan-600 dark:text-cyan-200" /><span className="text-sm font-black">EngVox Trust Center</span><span className="rounded-md border border-cyan-500/20 bg-cyan-500/10 px-1.5 py-px text-[9px] font-black text-cyan-700 dark:text-cyan-100">v{PRODUCT_VERSION}</span></div>
          <div className="flex items-center gap-1.5 rounded-2xl border border-slate-900/10 bg-white/70 p-1 dark:border-white/10 dark:bg-black/20">{(['privacy', 'terms'] as LegalDocument[]).map((docId) => (<button key={docId} onClick={() => setActiveDoc(docId)} className={`rounded-xl px-3 py-1.5 text-xs font-black capitalize transition-all cursor-pointer ${activeDoc === docId ? 'bg-slate-950 text-white dark:bg-white dark:text-slate-950' : 'text-slate-600 hover:text-slate-950 dark:text-white/65 dark:hover:text-white'}`}>{docId === 'privacy' ? 'Gizlilik' : 'Şartlar'}</button>))}</div>
        </div>
        <div className="rounded-[2rem] border border-slate-900/10 bg-white/76 p-6 shadow-2xl backdrop-blur-2xl dark:border-white/10 dark:bg-white/[0.07] sm:p-8">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3"><span className="inline-flex items-center gap-1.5 rounded-full border border-cyan-500/20 bg-cyan-500/10 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-cyan-700 dark:text-cyan-100"><Lock className="h-3 w-3" />{content.badge}</span><span className="text-xs font-bold text-slate-500 dark:text-white/50">Last Updated: {content.lastUpdated}</span></div>
          <h1 className="text-3xl font-black tracking-tight sm:text-5xl">{content.title}</h1>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-700 dark:text-slate-200">{content.summary}</p>
          <div className="mt-5 flex flex-wrap items-center gap-3 border-t border-slate-900/10 pt-5 text-xs font-bold text-slate-700 dark:border-white/10 dark:text-slate-200">{['Secure by design', 'User data ownership', 'Zero AI model training'].map((x) => (<span key={x} className="flex items-center gap-1.5"><CheckCircle2 className="h-4 w-4 text-cyan-600 dark:text-cyan-200" />{x}</span>))}</div>
        </div>
        <div className="mt-5 space-y-3">{content.sections.map(([title, text]) => (<section key={title} className="rounded-3xl border border-slate-900/10 bg-white/72 p-5 shadow-sm backdrop-blur-2xl transition-all hover:border-cyan-500/30 dark:border-white/10 dark:bg-white/[0.06]"><h2 className="text-base font-black tracking-tight">{title}</h2><p className="mt-2 text-sm leading-7 text-slate-700 dark:text-slate-200">{text}</p></section>))}</div>
        <div className="mt-8 flex items-center justify-between border-t border-slate-900/10 pt-5 text-xs font-bold text-slate-600 dark:border-white/10 dark:text-white/55"><span>EngVox © 2026</span><Link to="/" className="text-cyan-700 hover:underline dark:text-cyan-100">Ana sayfaya dön →</Link></div>
      </article>
