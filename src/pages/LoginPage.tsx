...
const LoginPage = () => {
  ...
  const handleSocialLogin = async (provider: 'google' | 'linkedin' | 'apple') => {
    ...
    try {
      ...
      if (authError) {
        if (authError.message?.includes('not enabled') || authError.message?.includes('Unsupported provider')) {
          setError(`${provider.charAt(0).toUpperCase() + provider.slice(1)} login is not yet enabled. Please use email login or try the demo mode.`);
        } else {
          throw authError;
        }
      }
          ...
          <div className="space-y-3">
            {SOCIAL_PROVIDERS.map((sp) => (
              ...
              >
                ...
                {socialLoading === sp.provider ? 'Connecting...' : `Continue with ${sp.name}`}
              </button>
            ))}
