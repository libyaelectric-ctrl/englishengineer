        ...
        <div className="grid grid-cols-2 gap-2 rounded-xl border border-border-soft bg-surface p-1.5 sm:grid-cols-4">
          {TABS.map((item) => (
            <button
              ...
              type="button"
              onClick={() => setTab(item)}
              className={`min-h-9 rounded-lg px-3 text-sm font-bold transition-all ${tab === item ? 'bg-primary text-white shadow-sm' : 'text-muted-copy hover:bg-primary/5 hover:text-foreground'}`}
            >
              {item}
            </button>
          ))}
            ...
            {visibleRules
              ...
              .map((rule) => (
                <button
                  ...
                  className={`shrink-0 px-4 py-2 rounded-lg text-sm font-semibold transition-all whitespace-nowrap ${selectedRule?.id === rule.id ? 'bg-primary text-white shadow-md' : 'bg-surface-hover text-muted-copy hover:bg-primary/10 hover:text-foreground'}`}
                >
