...
export const ProgressOverviewTab = () => {
  ...
  const chartTabs: Array<{ id: ReturnType<typeof useAnalyticsStore.getState>['activeChart']; label: string }> = [
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        <MetricCard label="Overall Progress" value={`${analytics.overallProgress}%`} icon={Target} trend={`${analytics.recentSessions.length} recent sessions`} statusColor="primary" />
        <MetricCard label="Study Consistency" value={`${analytics.studyConsistency}%`} icon={Calendar} trend={`${analytics.averageSessionLength}m avg session`} statusColor="cyan" />
        <MetricCard label="Improvement Velocity" value={`${analytics.improvementVelocity >= 0 ? '+' : ''}${analytics.improvementVelocity}%`} icon={TrendingUp} trend={`${analytics.retention}% retention signal`} statusColor={analytics.improvementVelocity >= 0 ? 'emerald' : 'amber'} />
        <MetricCard label="AI Coach Usage" value={`${analytics.aiCoachUsage.totalSessions}`} icon={Sparkles} trend={analytics.aiCoachUsage.mostUsedMode} statusColor="amber" />
      </div>

      {/* Assessment Profile */}
      <SectionCard title="Assessment Profile" subtitle="Engineering communication dimensions derived from existing learning evidence" icon={ShieldCheck}
        headerActions={<StatusBadge label={analytics.assessmentProfile.trustLabel} tone={analytics.assessmentProfile.hasEnoughData ? 'success' : 'warning'} />}>
      ...
      {advancedAnalyticsEntitlement.allowed && (
        <SectionCard title="Performance Command Center" subtitle="Derived from existing learning, vocabulary, achievement, and AI Coach state" icon={BarChart3}
          headerActions={
            <div className="flex flex-wrap gap-1 rounded-xl border border-border-soft bg-surface-hover p-1">
              {chartTabs.map((tab) => (
                <button key={tab.id} onClick={() => setActiveChart(tab.id)} className={`px-3 py-1.5 text-xs font-mono font-medium rounded-lg uppercase tracking-wider transition-all cursor-pointer ${activeChart === tab.id ? 'bg-primary text-white' : 'text-muted-copy hover:bg-surface-hover hover:text-primary'}`}>{tab.label}</button>
              ))}
            </div>
          }>
          <div className="pt-6">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {analytics.skillRadar.map((skill) => (
                    <div key={skill.module} className="rounded-xl border border-border-soft bg-surface-hover p-4">
                      <div className="flex items-center justify-between">
                        <h4 className="text-sm font-medium text-foreground">{skill.module}</h4>
                        <span className={`text-[10px] font-mono uppercase ${skill.trend === 'up' ? 'text-success' : skill.trend === 'down' ? 'text-rose-400' : 'text-muted-copy'}`}>{skill.trend}</span>
                      </div>
            ...
            {activeChart === 'xp' && <TimelinePanel title="XP Timeline" icon={Zap} points={analytics.xpTimeline} footer={`Growth delta: ${analytics.xpGrowth >= 0 ? '+' : ''}${analytics.xpGrowth} XP`} />}
            {activeChart === 'elo' && <TimelinePanel title="Skill Progress Timeline" icon={LineChart} points={analytics.eloTimeline} footer={`Index growth: ${analytics.eloGrowth >= 0 ? '+' : ''}${analytics.eloGrowth}`} />}
            {activeChart === 'vocabulary' && (
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <MiniStat label="Words Learned" value={`${analytics.vocabularySummary.wordsLearned}`} />
                  <MiniStat label="Reviews Today" value={`${analytics.vocabularySummary.todaysReviews}`} />
                  <MiniStat label="Vocab Streak" value={`${analytics.vocabularySummary.vocabularyStreak}d`} />
                </div>
                <AnalyticsProgress label="Vocabulary Retention" value={analytics.vocabularyRetention} />
                <div className="space-y-3">
                  {analytics.vocabularySummary.categoryMastery.map((item) => (
                    <AnalyticsProgress key={item.discipline} label={item.discipline} value={item.percentage} />
                  ))}
                </div>
              </div>
            )}
      ...
      {/* Recent Sessions */}
      <SectionCard title="Recent Sessions" subtitle="Latest learning activity across all engines" icon={Activity}>
        <div className="space-y-3">
          {analytics.recentSessions.map((session) => (
            <div key={`${session.timestamp}-${session.module}`} className="flex items-center justify-between rounded-xl border border-border-soft bg-surface-hover p-4">
              <div>
                <p className="text-sm font-medium text-foreground">{session.module}</p>
                <p className="text-[10px] font-mono text-muted-copy">{new Date(session.timestamp).toLocaleDateString()}</p>
              </div>
              <div className="text-right">
                <p className="text-sm font-medium text-primary">{session.score}%</p>
                <p className="text-[10px] font-mono text-muted-copy">{session.durationMinutes} min</p>
              </div>
            </div>
          ))}
          {analytics.recentSessions.length === 0 && <p className="text-xs text-muted-copy">No recent sessions yet.</p>}
        </div>
      </SectionCard>
