...
export const PersonalLeaderboard = ({ learningState }: { learningState: LearningState }) => {
  const rows = useMemo(() => {
  ...
  return (
    <SectionCard title="Personal Score History" subtitle="Last 7 days" icon={Trophy}>
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-border-soft">
              <th className="pb-2 text-left font-mono uppercase text-muted-copy">Date</th>
              <th className="pb-2 text-right font-mono uppercase text-muted-copy">XP</th>
              <th className="pb-2 text-right font-mono uppercase text-muted-copy">Tasks</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => (
