...
export const ProgressReport = ({
  ...
  <Card className="space-y-5 p-5 shadow-sm" hoverEffect={false}>
    <div className="flex items-center gap-3">
      ...
      <div>
        <h2 className="text-xl font-bold text-foreground">7-Day Progress Report</h2>
    ...
    <div className="grid gap-3 sm:grid-cols-3">
      <div className="rounded-[4px] border border-border-soft bg-surface p-3 shadow-sm">
        <p className="text-[10px] font-bold uppercase tracking-wider text-muted-copy">Work Tools</p>
        <p className="mt-1 text-sm font-medium text-foreground">{report.recommendedWorkTools}</p>
      </div>
      <div className="rounded-[4px] border border-border-soft bg-surface p-3 shadow-sm">
        <p className="text-[10px] font-bold uppercase tracking-wider text-muted-copy">Quick AI</p>
    ...
    <Button
      ...
      onClick={() => {
        markReportGenerated();
        BetaService.trackEvent('seven_day_report_generated', '/progress/next-steps');
      }}
