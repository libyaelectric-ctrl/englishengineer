...
export const PlanStep = ({ selectedPlan, setSelectedPlan }: PlanStepProps) => (
  <section>
    ...
    <p className="mt-2 text-sm text-muted-copy">
      Plan selection does not activate billing. Local mode remains Free until the backend verifies a
      subscription.
    </p>
    <div className="mt-6 grid gap-3 sm:grid-cols-3">
      {PLANS.map((plan) => (
        ...
        >
          ...
          <span className="mt-2 block text-xs leading-5 text-muted-copy">{plan.text}</span>
        </button>
      ))}
    </div>
  </section>
