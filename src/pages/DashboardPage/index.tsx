...
export const DashboardPage: React.FC = () => {
  ...
  return (
    <PageContainer className="max-w-6xl space-y-5">
      <header className="relative overflow-hidden rounded-[2rem] border border-slate-900/10 bg-white/70 p-6 shadow-2xl backdrop-blur-2xl dark:border-white/10 dark:bg-white/[0.07] sm:p-8">
        ...
        <div className="relative flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-cyan-700 dark:text-cyan-100">
            ...
            {currentUser?.email && (
              <p className="mt-1 text-sm font-bold text-slate-600 dark:text-white/55">
                {currentUser.email}
              </p>
            )}
            {meta && (
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3">
              <Link
                to="/curriculum"
                className="inline-flex items-center gap-1.5 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-black text-white transition hover:-translate-y-0.5 dark:bg-white dark:text-slate-950"
              >
              ...
              <Link
                to="/learning-path"
                className="inline-flex items-center gap-1.5 rounded-2xl border border-cyan-500/25 bg-cyan-500/10 px-5 py-3 text-sm font-black text-cyan-800 transition hover:-translate-y-0.5 dark:text-cyan-100"
              >
      ...
      <div className="rounded-[2rem] border border-slate-900/10 bg-white/68 p-5 shadow-sm backdrop-blur-2xl dark:border-white/10 dark:bg-white/[0.06]">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="flex items-center gap-2 text-sm font-black">
            <Settings className="h-4 w-4 text-cyan-700 dark:text-cyan-100" />
            {translate('dashboard.myDiscipline')}
          </h2>
          <Link
            to="/profile"
            className="flex items-center gap-1 text-xs font-black text-cyan-700 hover:underline dark:text-cyan-100"
          >
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex min-w-0 flex-1 items-center gap-3 sm:min-w-[200px]">
            <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-cyan-500/10 text-cyan-700 dark:text-cyan-100">
          ...
          <div className="hidden h-10 w-px bg-slate-900/10 dark:bg-white/10 sm:block" />
          <div className="flex min-w-0 flex-1 items-center gap-3 sm:min-w-[160px]">
            <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-blue-500/10 text-blue-600 dark:text-blue-200">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-3 sm:min-w-[140px]">
            <div className="grid h-11 w-11 place-items-center rounded-2xl bg-violet-500/10 text-violet-600 dark:text-violet-200">
            ...
            <div>
              ...
              <p className="text-sm font-black">{meta?.wordCount?.toLocaleString() ?? '—'}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {missions?.find((m) => m.personalReason) && (
        <div className="rounded-[2rem] border border-cyan-500/20 bg-cyan-500/10 px-4 py-3">
          <p className="text-xs font-black text-cyan-800 dark:text-cyan-100">
            {missions.find((m) => m.personalReason)?.personalReason}
          </p>
          <Link
            to="/curriculum"
            className="mt-1 inline-block text-xs font-bold text-slate-600 underline transition hover:text-cyan-700 dark:text-white/55 dark:hover:text-cyan-100"
          >
      ...
      <div className="rounded-[2rem] border border-slate-900/10 bg-white/68 p-5 shadow-sm backdrop-blur-2xl dark:border-white/10 dark:bg-white/[0.06]">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-sm font-black">{translate('dashboard.globalProgress')}</h2>
          <span className="text-xs font-black text-cyan-700 dark:text-cyan-100">{vocabBand}</span>
        </div>
        <div className="h-2 w-full overflow-hidden rounded-full bg-slate-900/10 dark:bg-white/10">
          <div
        ...
        <div className="mt-3 flex items-center justify-between text-xs font-bold text-slate-600 dark:text-white/55">
          <span>
