import { BookOpen, Headphones, Mic2, PenTool, BookMarked } from 'lucide-react';
...
const SKILL_LINKS = [
  { key: 'vocabulary', path: '/vocabulary', icon: BookMarked, color: 'bg-blue-500/10 text-blue-600 dark:text-blue-400' },
  { key: 'reading', path: '/reading', icon: BookOpen, color: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' },
  { key: 'writing', path: '/writing', icon: PenTool, color: 'bg-amber-500/10 text-amber-600 dark:text-amber-400' },
  { key: 'listening', path: '/listening', icon: Headphones, color: 'bg-cyan-500/10 text-cyan-600 dark:text-cyan-400' },
  { key: 'speaking', path: '/speaking', icon: Mic2, color: 'bg-rose-500/10 text-rose-600 dark:text-rose-400' },
];
...
export const ContentPoolWidget = ({ discipline }: ContentPoolWidgetProps) => {
  ...
  if (isLoading) {
    return (
      <div className="rounded-xl border border-border-soft bg-surface p-5 shadow-sm animate-pulse">
  ...
  if (error || !data) {
    return (
      <div className="rounded-xl border border-error/20 bg-error/5 p-5 shadow-sm">
        <p className="text-sm text-error font-bold">{translate('dashboard.contentError') ?? 'Unable to load content'}</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-5 gap-2">
        {SKILL_LINKS.map((skill) => (
          <Link
            key={skill.key}
            to={skill.path}
            className={`flex flex-col items-center gap-1.5 rounded-lg border p-3 transition-all hover:shadow-sm hover:scale-[1.02] ${skill.color}`}
          >
            <skill.icon className="h-5 w-5" />
            <span className="text-lg font-extrabold">{skillCounts[skill.key as keyof typeof skillCounts]}</span>
