...
import { OFFLINE_CAPABILITIES, getCapabilityLabel, getOfflineSummary } from '@/features/offline';
    ...
    <div className="space-y-7 animate-in fade-in duration-300 pt-12 sm:pt-0">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card hoverEffect={false}>
          ...
          <p className="mt-4 text-3xl font-medium text-foreground">{summary.available}</p>
          <p className="mt-1 text-sm text-muted-copy">Offline-available areas</p>
        </Card>
        <Card hoverEffect={false}>
          <History className="h-5 w-5 text-warning" />
          <p className="mt-4 text-3xl font-medium text-foreground">{summary.limited}</p>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {OFFLINE_CAPABILITIES.map((capability) => (
          <Card key={capability.id} className="flex items-start justify-between gap-4" hoverEffect>
            <div>
              <h2 className="font-medium text-foreground">{capability.name}</h2>
              <p className="mt-1 text-sm leading-6 text-muted-copy">{capability.description}</p>
            </div>
            <StatusBadge
      ...
      <Card className="space-y-4" hoverEffect={false}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-medium text-foreground">Available offline</h2>
            <p className="text-sm text-muted-copy">
              Core learning and saved progress remain available. Connected AI, cloud sync and
              billing require internet access.
            </p>
          </div>
        </div>
        ...
        <div>
          <p className="text-sm font-medium text-foreground">Recently used work-tool items</p>
          <p className="mt-1 text-sm text-muted-copy">
        ...
        <div>
          <p className="text-sm font-medium text-foreground">Recent dictionary searches</p>
          <p className="mt-1 text-sm text-muted-copy">
            {recentSearches.length ? recentSearches.join(', ') : 'No dictionary search saved yet.'}
          </p>
        </div>
        <div>
          <p className="text-sm font-medium text-foreground">Saved Quick AI draft</p>
          <p className="mt-1 text-sm text-muted-copy">
            {quickAIDraft ? quickAIDraft.title : 'No local Quick AI draft saved.'}
          </p>
        </div>
      </Card>
    </div>
