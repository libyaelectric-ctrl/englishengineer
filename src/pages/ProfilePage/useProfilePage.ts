...
export const useProfilePage = () => {
  ...
  const {
    subscription, providerStatus, isLoading: isBillingLoading,
    error: billingError, initializeBilling, refreshBilling,
    startCheckout, openCustomerPortal,
  } = useBillingStore();

  // UI state
  const [ui, dispatchUI] = useReducer(uiReducer, {
    isSaving: false, message: null as string | null,
    error: null as string | null, showClearConfirmation: false, clearConfirmation: '',
  ...
  const setClearConfirmation = (v: string) => dispatchUI({ type: 'SET_CLEAR_CONFIRMATION', value: v });
  ...
  const todaysReviews = useVocabularyStore.getState().history.filter(
  ...
  const voiceMinutesUsed = (() => {
    ...
    const totalSeconds = speakingHistory
      ...
      .reduce((sum, e) => sum + (e.evaluation?.wordsPerMinute
        ? Math.round((e.evaluation.wordCount / Math.max(e.evaluation.wordsPerMinute, 1)) * 60) : 0), 0);
  ...
  useEffect(() => {
    if (new URLSearchParams(location.search).get('billing') === 'success' && currentUser?.id) {
      ProductAnalyticsService.track('checkout_completed', '/profile', { metadata: { source: 'checkout_return' } });
  ...
  const handleUpgrade = async () => {
    ...
    if (currentUser.id.startsWith('demo_engineer_')) {
      setError('Demo mode: Billing is available after connecting Supabase and Stripe.'); return;
    }
    try {
      setError(null);
      ProductAnalyticsService.track('checkout_started', '/profile', { metadata: { plan: 'pro', source: 'user' } });
  ...
  const handleManageSubscription = async () => {
    if (!currentUser) return;
    if (currentUser.id.startsWith('demo_engineer_')) {
      setError('Demo mode: Subscription management available after connecting Supabase + Stripe.'); return;
    }
    try { setError(null); await openCustomerPortal(currentUser.id); }
    catch (e: unknown) { setError(e instanceof Error ? e.message : 'Billing portal is not available in demo mode.'); }
  };

  const exportLocalData = () => {
    const payload = { product: 'EngVox', version: '4.0.1', exportedAt: new Date().toISOString(), scope: 'Local EngVox data stored on this device', data: storage.exportAll() };
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url;
    a.download = `EngVox-local-data-${new Date().toISOString().slice(0, 10)}.json`; a.click();
    URL.revokeObjectURL(url); setMessage('Local EngVox data export created.'); setError(null);
  };

  const clearLocalData = async () => {
    if (providerMode !== 'local' || clearConfirmation !== 'CLEAR') return;
    await logout(); storage.clear(); window.location.assign('/start');
  };

  const resetLearningProgress = async () => {
    if (clearConfirmation !== 'CLEAR') return;
    ['learning_state', 'EngVox_vocabulary_menu', 'EngVox_grammar_progress', 'EngVox_reading_state',
      'EngVox_writing_state', 'EngVox_listening_state', 'EngVox_speaking_state', 'EngVox_vocabulary_state',
      'EngVox_vocabulary_memory', 'EngVox_placement_result', 'task_evaluation_records',
      'learning_intelligence', 'gamification_pro_state', 'streak_data', 'ai_coach_pro_state',
    ].forEach((k) => storage.remove(k));
    if (currentUser?.id) (await import('@/features/profile')).LearningProfileRepository.reset(currentUser.id);
    storage.clear(); window.location.assign('/start');
  };
  ...
  return {
    activeSection, currentUser, providerMode, subscription, providerStatus,
    isBillingLoading, billingError, profile, memory, learningState, mistakeLog,
    isSaving, message, error, showClearConfirmation, setShowClearConfirmation,
    clearConfirmation, setClearConfirmation, completionPercent,
    todaysCoachSessions, todaysAttempts, todaysReviews, uploadedDocsCount, voiceMinutesUsed,
    ...edit, ...prefs,
    handleUpgrade, handleManageSubscription, exportLocalData, clearLocalData, resetLearningProgress,
  };
};
