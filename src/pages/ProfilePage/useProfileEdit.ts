...
export const useProfileEdit = (
  ...
  const {
    isEditMode, firstName: editFirstName, lastName: editLastName,
    profession: editProfession, track: editTrack, subdomain: editSubdomain,
    industry: editIndustry, lang: editLang, goals: editGoals,
  } = edit;
  ...
  const handleSaveProfile = async (event: React.FormEvent) => {
    ...
    try {
      ...
      LearningProfileRepository.updatePreferences(currentUser?.id ?? 'local-user', {
        ...
        electricalSubdomain: (editSubdomain as UserLearningProfile['electricalSubdomain']) || undefined,
      ...
      setIsEditMode(false);
    } catch (e: unknown) { setError(getErrorMessage(e, 'Failed to update profile overview.')); }
  };

  return {
    isEditMode, editFirstName, editLastName, editProfession, editTrack,
    editSubdomain, editIndustry, editLang, editGoals,
    setEditFirstName, setEditLastName, setEditProfession, setEditTrack,
    setEditSubdomain, setEditIndustry, setEditLang, setEditGoals, setIsEditMode,
    enterEditMode, handleSaveProfile,
  };
};
