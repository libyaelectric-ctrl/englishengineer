...
const readInitialSession = (): PersistedLocalSession | null => {
  ...
  if (!saved || !['local', 'demo'].includes(saved.kind) || !saved.user?.id) { storage.globalRemove('auth_user'); return null; }
...
export const useAuthStore = create<AuthState & AuthActions>()(devtools((set) => ({
  ...
  loginAsLocal: (userData) => { const email = userData.email.trim().toLowerCase(); const userId = `eng_${email.replace(/[^a-zA-Z0-9]/g, '_')}`; const displayName = userData.displayName || email.split('@')[0] || 'Engineer'; const profile: UserProfile = { id: userId, displayName, email, role: 'engineer', isSuperUser: false, engineeringDiscipline: userData.discipline || 'electrical', targetLevel: 'B2', location: '', avatarInitials: displayName.slice(0, 2).toUpperCase(), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }; activateProfile(profile, 'local'); set({ currentUser: profile, isAuthenticated: true, isLoading: false, sessionKind: 'local' }); return profile; },
  enterDemoUser: () => { const profile: UserProfile = { id: `demo_engineer_${Date.now()}`, displayName: 'Demo Engineer', email: 'demo@engvox.com', role: 'engineer', isSuperUser: false, engineeringDiscipline: 'electrical', targetLevel: 'B2', location: '', avatarInitials: 'DE', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }; activateProfile(profile, 'demo'); set({ currentUser: profile, isAuthenticated: true, isLoading: false, sessionKind: 'demo' }); return profile; },
  logout: async () => { set({ isLoading: true }); const providerSignOut = useAuthStore.getState().providerSignOut; try { if (providerSignOut) await providerSignOut(); } catch (error) { logger.e('Provider sign-out failed; local session will still be cleared.', error); } finally { storage.deactivateSession(); storage.globalRemove(SESSION_KEY); storage.globalRemove('auth_user'); set({ currentUser: null, isAuthenticated: false, isLoading: false, sessionKind: null, providerUserSync: null, providerSignOut: null }); } },
