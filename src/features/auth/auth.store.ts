...
import { type ClientSessionKind, storage } from '@/shared/storage';
...
const readInitialSession = (): PersistedLocalSession | null => {
  ...
  if (!saved || !['local', 'demo'].includes(saved.kind) || !saved.user?.id) {
    if (saved) storage.globalRemove(SESSION_KEY);
    return null;
  }
...
const persistLocalSession = (kind: LocalSessionKind, user: UserProfile): void => {
  storage.globalSet(SESSION_KEY, { kind, user } satisfies PersistedLocalSession);
};
...
export const useAuthStore = create<AuthState & AuthActions>()(
  devtools(
    (set) => ({
      ...
      isLoading: false,
      sessionKind: initialSession?.kind ?? null,
      providerUserSync: null,
      providerSignOut: null,
      loginAsLocal: (userData) => {
        const email = userData.email.trim().toLowerCase();
        const userId = `eng_${email.replace(/[^a-zA-Z0-9]/g, '_')}`;
        const displayName = userData.displayName || email.split('@')[0] || 'Engineer';
      ...
      logout: async () => {
        ...
        try {
          if (providerSignOut) await providerSignOut();
        } catch (error) {
          logger.e('Provider sign-out failed; local session will still be cleared.', error);
        } finally {
          setAuthTokenGetter(null);
          storage.deactivateSession();
          storage.globalRemove(SESSION_KEY);
          storage.globalRemove('auth_user');
          set({
            currentUser: null,
      ...
      updateProfile: async (updates) => {
        ...
        const kind = useAuthStore.getState().sessionKind;
        if (kind === 'local' || kind === 'demo') persistLocalSession(kind, updated);
        const sync = useAuthStore.getState().providerUserSync;
        if (sync && updates.displayName) {
          try {
            await sync(updates);
          } catch (error) {
            logger.w('Auth provider profile sync failed.', error);
          }
        }
      },
      setProviderUserSync: (providerUserSync) => set({ providerUserSync }),
      setProviderSignOut: (providerSignOut) => set({ providerSignOut }),
    }),
    { name: 'AuthStore' }
  )
);
