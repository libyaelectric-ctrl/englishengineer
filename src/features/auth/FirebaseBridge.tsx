...
const buildProfile = async (
  ...
  try {
    ...
    role = typeof token.claims.role === 'string' ? token.claims.role : null;
    isSuperUser = token.claims.isSuperUser === true || role === 'Super Administrator';
  } catch (err) {
    logger.e('FirebaseBridge offline claim error:', err);
  }
  return {
    id: user.uid,
    ...
    targetLevel: '',
    location: '',
    avatarInitials: initials(user.displayName || '', email),
    createdAt: new Date(user.metadata.creationTime || Date.now()).toISOString(),
    updatedAt: new Date(user.metadata.lastSignInTime || Date.now()).toISOString(),
  };
};
export const FirebaseBridge = () => {
  ...
  useEffect(() => {
    ...
    if (isSignedIn === false) {
      ...
      if (state.sessionKind === 'firebase' || bridgedUserId.current) {
