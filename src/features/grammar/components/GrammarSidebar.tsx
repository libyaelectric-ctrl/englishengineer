...
export function GrammarSidebar() {
  ...
  const config: SidebarConfig = {
    ...
    tabs: (['New', 'Learning', 'Due', 'Strong'] as const).map((t) => ({
      label: t,
      active: t === tab,
      badge: t === 'New' ? g.newRules : t === 'Learning' ? g.learning : t === 'Due' ? g.due : g.strong,
      onClick: () => { setTab(t); log('/grammar', 'tab', t); },
    })),
    stats: [
    ...
    progressBars: [
      { label: 'Strong', value: g.strong, max: 360, color: '#8b5cf6', showPercent: true },
      { label: 'Due', value: g.due, max: 360, color: '#e879f9' },
    ],
    actions: [
      { icon: '📝', label: `Practice ${g.due} due rules`, onClick: () => log('/grammar', 'practice', `${g.due} due`), variant: 'warning' },
      { icon: '🔄', label: 'Review strong', onClick: () => log('/grammar', 'review', 'strong') },
    ],
    custom: selectedRule ? (
      <div className="rounded-lg bg-surface-hover p-3 border border-border-soft">
        <p className="text-[10px] font-bold text-primary mb-1">LESSON {selectedRuleIndex + 1} OF {rules.length}</p>
