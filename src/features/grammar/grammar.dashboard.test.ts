...
const makeProgress = (id: string, overrides: Partial<GrammarRuleProgress> = {}): GrammarRuleProgress => ({
  ruleId: id,
  exposures: 5,
  correctUsages: 3,
  incorrectUsages: 1,
...
describe('GrammarDashboardService', () => {
  describe('buildDashboard', () => {
    it('calculates overall progress', () => {
      ...
      const errorSummary = { totalErrors: 5, topCategories: [] as Array<{ category: ErrorCategory; count: number; percentage: number }> };
      const dashboard = GrammarDashboardService.buildDashboard(progress, errorSummary, () => 'tense');
    ...
    it('builds category breakdown', () => {
      ...
      const errorSummary = { totalErrors: 0, topCategories: [] as Array<{ category: ErrorCategory; count: number; percentage: number }> };
      const dashboard = GrammarDashboardService.buildDashboard(progress, errorSummary, () => 'tense');
    ...
    it('generates recommendations', () => {
      ...
      const errorSummary = { totalErrors: 10, topCategories: [{ category: 'tense' as ErrorCategory, count: 8, percentage: 80 }] };
      const dashboard = GrammarDashboardService.buildDashboard(progress, errorSummary, () => 'tense');
