...
const MemoryEntryItem = ({
  keyName, value, isEditing, editValue, setEditValue,
  hasProjectAccess, onSave, onCancel, onStartEdit, onDelete,
  ...
  <li className="flex items-start gap-2 rounded-[4px] border border-[#d9d9e3] bg-[#faf8ff] p-2 shadow-sm">
    {isEditing ? (
      <div className="flex flex-1 flex-col gap-1.5">
        ...
        <textarea id={`memory-edit-${keyName}`} value={editValue}
      ...
      <>
        <div className="flex-1 min-w-0">
          <p className="text-[10px] font-bold text-[#0047bb] truncate">{keyName}</p>
          <p className="mt-0.5 text-xs leading-4 text-foreground font-semibold break-words">{value}</p>
        </div>
        {hasProjectAccess && (
          <div className="flex shrink-0 gap-1">
            <button type="button" onClick={onStartEdit} aria-label={`Edit ${keyName}`}
            ...
            <button type="button" onClick={onDelete} aria-label={`Delete ${keyName}`}
...
const AddMemoryForm = ({
  newKey, newValue, setNewKey, setNewValue, keyError, onAdd, onCancel,
}: {
  newKey: string; newValue: string;
  setNewKey: (v: string) => void; setNewValue: (v: string) => void;
  keyError: string | null; onAdd: () => void; onCancel: () => void;
}) => (
  <div className="mt-2 rounded-[4px] border border-[#0047bb]/25 bg-[#0047bb]/5 p-3 space-y-2 shadow-sm">
    <p className="text-[10px] font-bold uppercase tracking-wider text-[#0047bb]">New Memory Entry</p>
    <input id="memory-new-key" type="text" placeholder="Key (e.g. Project Name, Client, Standards)"
      value={newKey} onChange={(e) => setNewKey(e.target.value)} aria-label="Memory entry key"
    ...
    <label className="block">
      ...
      <textarea id="memory-new-value" placeholder="Value (e.g. Offshore Wind Project Phase 2, IEC 61400 standard)"
        value={newValue} onChange={(e) => setNewValue(e.target.value)} rows={2}
    ...
    {keyError && (
      <p className="text-[10px] text-error font-bold uppercase tracking-wider leading-4">{keyError}</p>
    )}
    <div className="flex gap-1.5">
      <button type="button" id="memory-add-confirm-btn" onClick={onAdd}
...
const WorkspaceMemoryContent = ({
  entries, editingKey, editValue, setEditValue, hasProjectAccess,
  addingNew, newKey, newValue, setNewKey, setNewValue, keyError,
  saveEditing, startEditing, cancelEditing, handleDelete, validateAndAdd, resetAddForm,
}: {
  ...
  <>
    {entries.length === 0 && !addingNew && (
      <div className="rounded-[4px] border border-dashed border-[#d9d9e3] bg-[#faf8ff] px-4 py-5 text-center shadow-sm">
        <p className="text-[10px] text-muted-copy font-medium">No memory entries yet. Add project context, standards, or team info.</p>
      </div>
    )}
    {entries.length > 0 && (
      <ul className="space-y-1.5 mb-2" aria-label="Memory entries">
        {entries.map(([key, value]) => (
          <MemoryEntryItem key={key} keyName={key} value={value}
            isEditing={editingKey === key} editValue={editValue} setEditValue={setEditValue}
            hasProjectAccess={hasProjectAccess} onSave={saveEditing}
            onCancel={cancelEditing} onStartEdit={() => startEditing(key, value)}
            onDelete={() => handleDelete(key)} />
        ))}
      </ul>
    )}
    {addingNew && (
      <AddMemoryForm newKey={newKey} newValue={newValue} setNewKey={setNewKey}
        setNewValue={setNewValue} keyError={keyError} onAdd={validateAndAdd} onCancel={resetAddForm} />
    )}
...
export const WorkspaceMemoryPanel = ({
  ...
  const validateAndAdd = () => {
    ...
    if (!k) { setKeyError('Key cannot be empty.'); return; }
    if (memory[k] !== undefined) { setKeyError(`"${k}" already exists. Edit it instead.`); return; }
    if (!v) { setKeyError('Value cannot be empty.'); return; }
  ...
  const resetAddForm = () => { setAddingNew(false); setNewKey(''); setNewValue(''); setKeyError(null); };
  const startEditing = (key: string, val: string) => { setEditingKey(key); setEditValue(val); };
  const saveEditing = () => {
    ...
    <div className="rounded-[4px] border border-[#d9d9e3] bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between mb-3 border-b border-[#d9d9e3] pb-2">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[10px] font-bold uppercase tracking-wider text-foreground">Workspace Memory</p>
          {entries.length > 0 && (
        ...
        {!addingNew && hasProjectAccess && (
          <button type="button" id="workspace-memory-add-btn"
            onClick={() => { setAddingNew(true); setKeyError(null); }}
      ...
      <p className="text-[10px] leading-4 text-muted-copy mb-3 font-medium">
        Memory entries are automatically included in every AI request for this workspace. Use them to store project context, team names, standards, or preferences.
      </p>
      <WorkspaceMemoryContent
        entries={entries} editingKey={editingKey} editValue={editValue}
        setEditValue={setEditValue} hasProjectAccess={hasProjectAccess}
        addingNew={addingNew} newKey={newKey} newValue={newValue}
        setNewKey={setNewKey} setNewValue={setNewValue} keyError={keyError}
        saveEditing={saveEditing} startEditing={startEditing}
        cancelEditing={() => setEditingKey(null)}
        handleDelete={handleDelete} validateAndAdd={validateAndAdd}
