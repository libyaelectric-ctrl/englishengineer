...
interface WorkspaceStoreState {
  ...
  addDocumentToWorkspace: (id: string, name: string, content: string) => void;
  deleteDocumentFromWorkspace: (id: string, documentId: string) => void;
  ...
  persist((set, get) => {
    ...
    return {
      ...
      createWorkspace: (name, planId) => {
        ...
        if (workspaces.length >= getPlanWorkspaceLimit(planId)) return false;
      ...
      deleteWorkspace: (id) => {
        ...
        const workspaces = current.filter((workspace) => workspace.id !== id);
        const activeWorkspaceId =
          get().activeWorkspaceId === id ? workspaces[0].id : get().activeWorkspaceId;
        set({ workspaces, activeWorkspaceId });
        useAIStore
          .getState()
          .setSessions(
            workspaces.find((workspace) => workspace.id === activeWorkspaceId)?.sessions ?? []
          );
      },
      switchWorkspace: (id) => {
        const activeId = get().activeWorkspaceId;
        if (activeId === id || !get().workspaces.some((workspace) => workspace.id === id)) return;
        const sessions = useAIStore.getState().sessions;
        const workspaces = get().workspaces.map((workspace) =>
          workspace.id === activeId ? { ...workspace, sessions } : workspace
        );
        set({ workspaces, activeWorkspaceId: id });
        useAIStore
          .getState()
          .setSessions(workspaces.find((workspace) => workspace.id === id)?.sessions ?? []);
      },
      updateWorkspaceMemory: (id, key, value) =>
        set({
          workspaces: get().workspaces.map((workspace) =>
            workspace.id === id
              ? { ...workspace, memory: { ...workspace.memory, [key]: value } }
              : workspace
          ),
        }),
      addDocumentToWorkspace: (id, name, content) => {
        ...
        set({
          workspaces: get().workspaces.map((workspace) =>
            workspace.id === id
              ? { ...workspace, documents: [...workspace.documents, document] }
              : workspace
          ),
        });
      },
      deleteDocumentFromWorkspace: (id, documentId) =>
        set({
          workspaces: get().workspaces.map((workspace) =>
            workspace.id === id
              ? {
      ...
      resetWorkspaces: () => {
        ...
        set({ workspaces: [workspace], activeWorkspaceId: workspace.id });
        useAIStore.getState().setSessions([]);
      },
    };
