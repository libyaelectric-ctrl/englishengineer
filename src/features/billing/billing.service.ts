...
export const BillingService = {
  ...
  async fetchInvoices(userId: string): Promise<InvoiceRecord[]> { const provider = getProvider(); if (!provider) return []; try { return await provider.getInvoices(userId); } catch (err) { logger.e('[BILLING] Failed to fetch invoices:', err); return []; } },
};
