...
const PHRASE_TOPICS = [
  ...
  ['Contractor', 'the agreed corrective action', 'mutabık kalınan düzeltici faaliyet'],
  ['Client', 'the milestone decision', 'kilometre taşı kararı'],
  ['Subcontractor', 'the committed completion date', 'taahhüt edilen bitiş tarihi'],
  ['QA/QC', 'the inspection close-out evidence', 'kontrol kapanış kanıtı'],
  ['HSE', 'the immediate safety control', 'acil güvenlik kontrolü'],
  ['Procurement', 'the confirmed delivery programme', 'teyit edilmiş teslimat programı'],
  ['Commissioning', 'the outstanding test prerequisite', 'eksik test ön koşulu'],
  ...
  ['Clarification', 'the conflicting interface requirement', 'çelişen arayüz gereksinimi'],
  ...
  ['Warning', 'the repeated site non-compliance', 'tekrarlanan saha uygunsuzluğu'],
  ['Progress', 'the current installation status', 'mevcut montaj durumu'],
  ['Coordination', 'the unresolved coordination interface', 'çözülmemiş koordinasyon arayüzü'],
...
const PHRASE_PATTERNS = [
  {
    phrase: (topic: string) => `Please confirm ${topic} before we proceed with the next activity.`,
    turkish: (topic: string) => `Bir sonraki faaliyete geçmeden önce ${topic} teyit edin.`,
  ...
  },
  {
    ...
    turkish: (topic: string) => `${topic} için mevcut durum bugünkü aksiyon kaydına işlenmelidir.`,
    use: 'Keeping an auditable action record',
    example: (topic: string) =>
      `The current status of ${topic} needs to be recorded with an owner and due date.`,
  },
  {
    phrase: (topic: string) => `We can close this point once ${topic} is formally accepted.`,
    turkish: (topic: string) => `${topic} resmen kabul edildiğinde bu maddeyi kapatabiliriz.`,
    use: 'Explaining a closure condition',
    example: (topic: string) =>
      `We can close this point once ${topic} is formally accepted by the consultant.`,
  },
  {
    phrase: (topic: string) => `Could you identify the owner and deadline for ${topic}?`,
    turkish: (topic: string) => `${topic} için sorumluyu ve son tarihi belirtebilir misiniz?`,
  ...
  {
    phrase: (topic: string) => `Without ${topic}, the planned sequence remains at risk.`,
    turkish: (topic: string) => `${topic} olmadan planlanan iş sırası risk altında kalır.`,
...
export const EXPANDED_PHRASE_LIBRARY: PhraseEntry[] = PHRASE_TOPICS.flatMap(
  ([category, englishTopic, turkishTopic], categoryIndex) =>
    PHRASE_PATTERNS.map((pattern, patternIndex) => ({
      ...
      tags: [category.toLowerCase(), 'site meeting', 'engineering communication'],
    }))
);
