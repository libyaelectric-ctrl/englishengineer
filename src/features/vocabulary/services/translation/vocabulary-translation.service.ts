...
type LanguageMap = Record<string, TermTranslation>;
type TranslationMap = Record<string, LanguageMap>;
...
export const loadVocabularyTranslations = (): Promise<TranslationMap> => {
  if (!pending) {
    pending = import('../../../../../data/translations/vocabulary-translations.json').then(
      (mod) => {
