...
import { isTurkishQuizAnswerCorrect, selectRandomQuizItems } from './learned-quiz';
...
describe('learned vocabulary quiz helpers', () => {
  ...
  it('accepts Turkish input despite case, whitespace, and accents', () => {
    ...
    expect(isTurkishQuizAnswerCorrect('bus   hattı', 'bara / bus hattı')).toBe(true);
  });

  it('accepts a configured slash or comma meaning alternative', () => {
    expect(isTurkishQuizAnswerCorrect('şalter', 'anahtar / şalter')).toBe(true);
