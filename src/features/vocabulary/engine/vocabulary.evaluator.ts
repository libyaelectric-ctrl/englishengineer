...
export const VocabularyEvaluator = {
  evaluate(
    ...
    const speed = Math.round(
      Math.max(
        0,
        Math.min(MAX_SPEED_SCORE, MAX_SPEED_SCORE - averageSpeed * SPEED_DECAY_MULTIPLIER)
      )
    );
    ...
    const finalScore = Math.round(
      accuracy * FINAL_ACCURACY_WEIGHT +
        speed * FINAL_SPEED_WEIGHT +
        retention * FINAL_RETENTION_WEIGHT
    );
