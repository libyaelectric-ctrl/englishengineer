 ...
 *
 * Those counts are a property of the corpus, not of the code, so the committed fixture slices
 * cannot stand in for it: run with `ENGVOX_TEST_SEED_DIR` pointing at a real corpus copy, or this
 * suite skips.
 */
describe.skipIf(!hasSeedCorpus())(
  `curriculum.service (real vocabulary database) ${SEED_CORPUS_REQUIRED}`,
  () => {
