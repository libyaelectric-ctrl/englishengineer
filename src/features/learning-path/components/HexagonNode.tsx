...
export const HexagonNode = ({ level, onSelect }: HexagonNodeProps) => {
  ...
  return (
    ...
    >
      {locked ? (
        <Lock className="h-5 w-5 text-on-solid/80" />
      ) : (
        <span className="text-lg font-extrabold text-on-solid drop-shadow-sm">
          {level.index + 1}
        </span>
      )}
      {level.masteryRatio > 0 && level.status !== 'completed' && (
        <span className="absolute inset-x-2 top-1 h-0.5 rounded-full bg-on-solid/70" />
      )}
    </button>
  );
};
