import { AlertCircle, CheckCircle2, Loader2, MailPlus, Send } from 'lucide-react';
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  ...
                  Assign {parsedEmails.length > 0 ? `${parsedEmails.length} ` : ''}
                  Licenses
                </>
              )}
            </Button>
          ...
          <div className="space-y-2 rounded-[var(--radius-card)] border border-border-soft p-4">
            {result.succeeded.length > 0 && (
              <div className="flex items-start gap-2 text-sm text-success">
                ...
                <span>
                  ...
                  {result.succeeded.length !== 1 ? 's' : ''} created successfully.
                </span>
              </div>
            )}
            {result.failed.length > 0 && (
              <div className="space-y-1">
                {result.failed.map((f) => (
                  <div key={f.email} className="flex items-start gap-2 text-sm text-error">
