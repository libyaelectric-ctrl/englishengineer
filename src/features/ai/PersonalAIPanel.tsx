...
import { PersonalAIService, type PersonalLessonContent, type GenerateLessonParams } from './personal-ai.service';
...
export const PersonalAIPanel = ({ discipline, cefrLevel = 'B1', userName }: PersonalAIPanelProps) => {
    ...
    >
      <div className="space-y-4">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 rounded-xl bg-primary py-3 text-sm font-semibold text-primary-foreground disabled:opacity-50 cursor-pointer transition-all hover:bg-primary/90"
        >
        ...
        {error && (
          <div className="rounded-lg bg-rose-50 dark:bg-rose-950/30 p-3 text-xs text-rose-700 dark:text-rose-300">
            {error}
          </div>
        )}
              ...
              <div className="space-y-2">
                ...
                <div className="grid gap-2">
                  {lesson.vocabulary.map((v, i) => (
                    <div key={i} className="rounded-lg bg-surface-hover p-3 text-xs space-y-1">
