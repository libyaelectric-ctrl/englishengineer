...
export const useWritingStore = create<WritingStoreState & WritingStoreActions>((set, get) => ({
  ...
  initializeStore: () => {
    ...
    const lastId = state.lastSelectedMissionId || 'writing_a1_simple_site_update';
  ...
  submitCurrentMission: () => {
    ...
    const { selectedMissionId, draft, timeSpentSeconds, autoFixesUsed, missions } = get();
