...
const severityConfig: Record<RealtimeSuggestion['severity'], { icon: typeof CheckCircle2; color: string; bg: string }> = {
  error: { icon: AlertTriangle, color: 'text-rose-500', bg: 'bg-rose-500/5 border-rose-500/20' },
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="rounded-lg border border-border-soft bg-surface p-3 text-center">
          ...
          <p className={cn(
            'text-lg font-bold',
            analysis.readabilityScore >= 80 ? 'text-emerald-500' :
            analysis.readabilityScore >= 60 ? 'text-amber-500' : 'text-rose-500'
          )}>
      ...
      <div className="space-y-2">
        ...
        <ScoreBar label="Passive Voice" score={Math.max(0, 100 - analysis.passiveVoiceCount * 10)} />
      </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {suggestion.line && (
                        <span className="text-[10px] text-muted-copy">Line {suggestion.line}</span>
                      )}
                    </div>
                    <p className="mt-0.5 text-foreground">{suggestion.message}</p>
                    {suggestion.fix && (
...
const ScoreBar = ({ label, score }: { label: string; score: number }) => (
  <div className="flex items-center gap-3">
    <span className="w-20 text-[10px] font-mono text-muted-copy uppercase">{label}</span>
    ...
    <span className="w-8 text-[10px] font-mono text-muted-copy text-right">{score}</span>
  </div>
);
