...
describe('PronunciationFeedbackEngine', () => {
  describe('analyzeWord', () => {
    it('returns high accuracy for correct pronunciation', () => {
      const result = PronunciationFeedbackEngine.analyzeWord('hello', '/hɛloʊ/', 'hello');
      expect(result.overallAccuracy).toBe(100);
      expect(result.word).toBe('hello');
    });

    it('returns lower accuracy for mispronounced word', () => {
      const result = PronunciationFeedbackEngine.analyzeWord('hello', '/hɛloʊ/', 'helo');
      expect(result.overallAccuracy).toBeLessThan(100);
    });

    it('generates a tip', () => {
      const result = PronunciationFeedbackEngine.analyzeWord('think', '/θɪŋk/', 'tink');
      expect(result.tip).toBeTruthy();
      expect(typeof result.tip).toBe('string');
    });

    it('extracts phoneme details', () => {
      const result = PronunciationFeedbackEngine.analyzeWord('cat', '/kæt/', 'cat');
  ...
  describe('calculateSimilarity', () => {
    it('returns 1 for identical strings', () => {
      expect(PronunciationFeedbackEngine.calculateSimilarity('hello', 'hello')).toBe(1);
    });

    it('returns high value for similar strings', () => {
      expect(PronunciationFeedbackEngine.calculateSimilarity('hello', 'helo')).toBeGreaterThan(0.7);
    });

    it('returns low value for different strings', () => {
      expect(PronunciationFeedbackEngine.calculateSimilarity('cat', 'dog')).toBeLessThan(0.5);
    });
  });

  describe('getAccentLabel', () => {
    it('returns correct labels', () => {
      expect(PronunciationFeedbackEngine.getAccentLabel(95)).toBe('Native-like');
  ...
  describe('getPhonemeTip', () => {
    ...
    it('returns default tip for unknown phoneme', () => {
      expect(PronunciationFeedbackEngine.getPhonemeTip('x')).toContain('Practice');
    });
  });
});
