...
export const useReadingStore = create<ReadingStoreState & ReadingStoreActions>((set, get) => ({
  ...
  initializeStore: () => {
    ...
    set({
      selectedMissionId: state.lastSelectedMissionId || 'reading_a1_site_signs',
  ...
  submitCurrentMission: () => {
    ...
    const { selectedMissionId, answers, timeSpentSeconds, clickedVocab, missions } = get();
    ...
    try {
      ...
      void KnowledgeCaptureService.capture({
        ...
        vocabularyTerms: [...(mission?.vocabulary.map((item) => item.term) ?? []), ...clickedVocab],
