...
describe('KnowledgeCaptureService', () => {
  ...
  it('connects skill vocabulary and grammar exposure to Learning Memory', async () => {
    const vocabulary = (await VocabularyRepository.getVocabularyByLevel('A1'))[0];
    ...
    expect(VocabularyMenuService.getState().progress[vocabulary.id]?.status).toBe('Learned');
  ...
  it('never downgrades an existing mastered vocabulary item', async () => {
    ...
    expect(VocabularyMenuService.getState().progress[vocabulary.id]?.status).toBe('Mastered');
  });
});
