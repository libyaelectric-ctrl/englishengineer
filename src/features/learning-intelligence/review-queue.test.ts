...
describe('UnifiedReviewQueueService', () => {
  ...
  it('combines weak vocabulary, due grammar, repeated mistakes and skill opportunity', async () => {
    ...
    LearningIntelligenceService.addMistake('Grammar', 'Repeated tense', 'Use simple past.', start);
    LearningIntelligenceService.addMistake('Grammar', 'Repeated tense', 'Use simple past.', start);
    LearningIntelligenceService.addMistake('Grammar', 'Repeated tense', 'Use simple past.', start);
    ...
    const queue = await UnifiedReviewQueueService.buildQueue(profile, reviewDate);
    expect(queue.map((item) => item.source)).toEqual(
      expect.arrayContaining(['weak-word', 'due-item', 'repeated-mistake', 'skill-weakness'])
    );
