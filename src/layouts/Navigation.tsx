...
const linkClasses = ({ isActive, collapsed }: { isActive: boolean; collapsed?: boolean }) =>
  cn(
    'group relative flex min-h-10 items-center gap-2.5 rounded-[var(--radius-button)] text-sm font-semibold transition-all duration-150',
...
function CollapsedTooltip({ label }: { label: string }) {
  return (
    <span
      role="tooltip"
      className="pointer-events-none absolute left-full top-1/2 z-50 ml-3 -translate-y-1/2 whitespace-nowrap rounded-[var(--radius-button)] border border-border-soft bg-surface px-2.5 py-1 text-[11px] font-bold text-foreground shadow-pop opacity-0 transition-opacity duration-150 group-hover:opacity-100"
    >
      {label}
...
export const Navigation = React.memo(({ onItemClick, collapsed }: NavigationProps) => {
  ...
  const renderLockedLink = (item: LockableItem) => (
    <button
      ...
      className={cn(
        'group relative flex min-h-10 w-full cursor-pointer items-center gap-2.5 rounded-[var(--radius-button)] text-sm font-semibold text-muted-copy transition-all hover:bg-surface-hover hover:text-foreground',
        collapsed ? 'justify-center px-0 py-2' : 'px-3 py-2'
      )}
        ...
        return (
          <div key={item.label} className="space-y-1">
            <button
              ...
              className="group flex min-h-10 w-full items-center gap-2.5 rounded-[var(--radius-button)] px-3 py-2 text-sm font-semibold text-muted-copy transition-all hover:bg-surface-hover hover:text-foreground"
            ...
            >
              ...
              <ChevronDown
                className={cn(
                  'h-3.5 w-3.5 text-muted-copy transition-transform',
                  isOpen && 'rotate-180'
                )}
              />
            </button>
            {isOpen && 'children' in item && (
              <div className="ml-4 space-y-1 border-l border-border-soft pl-3">
