...
export const storage = {
  ...
  isAvailable(): boolean { try { if (typeof window === 'undefined' || !window.localStorage) return false; const key = '__eos_storage_test__'; localStorage.setItem(key, '1'); localStorage.removeItem(key); return true; } catch (err) { logger.e('[STORAGE] localStorage availability check failed:', err); return false; } },
