...
describe('storage namespace controls', () => {
  beforeEach(() => {
    ...
    storage.activateSession({ userId: 'test-user', kind: 'firebase' });
  });
  it('exports only the active session namespace', () => {
    storage.set('learning_state', { xp: 120 });
    localStorage.setItem('unrelated_key', 'team');
    expect(storage.exportAll()).toMatchObject({ learning_state: { xp: 120 } });
    expect(storage.exportAll()).not.toHaveProperty('unrelated_key');
  });
