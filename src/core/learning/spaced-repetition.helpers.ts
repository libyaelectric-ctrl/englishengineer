...
export const isCefrAtOrBelow = (candidate: CefrLevel, ceiling: CefrLevel): boolean =>
  CEFR_LEVELS.indexOf(candidate) <= CEFR_LEVELS.indexOf(ceiling);

export const includesNormalized = (values: string[], expected: string): boolean =>
  values.some((value) => value.trim().toLowerCase() === expected.trim().toLowerCase());
