  ...
  const checkDir = (dir: string): boolean => {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        if (importBlocks.some((block) => new RegExp(`\\b${symbol}\\b`).test(block))) return true;
      }
    }
    return false;
  };
...
describe('Architecture Rules', () => {
  it('core does not import from features', () => {
    ...
    const checkDir = (dir: string): void => {
      for (const entry of readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) checkDir(fullPath);
          ...
          for (const imp of getImportsFromFile(fullPath)) {
  ...
  it('documents feature-to-feature import violations (technical debt)', () => {
    ...
    expect(violations.length).toBeLessThan(52);
  });
  ...
  it('most features have an index.ts entry point', () => {
    ...
    for (const feature of featureDirs) {
      ...
      if (!existsSync(indexPath) && !existsSync(tsxPath)) missing.push(feature);
    }
    ...
    for (const block of exportBlocks) {
      for (const raw of block
        ...
        const symbol = raw
          ...
          .trim();
        if (!symbol) continue;
        if (!isCoreSymbolImported(symbol)) unused.push(symbol);
      }
    }
