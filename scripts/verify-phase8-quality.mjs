...
assert.doesNotMatch(idService, /\brequire\s*\(/, 'IdService must not use CommonJS require');
assert.doesNotMatch(
  idService,
  /Math\.random/,
  'IdService must not use non-cryptographic randomness'
);
...
assert.equal(new Set(keys).size, keys.length, '.env.example contains duplicate keys');
...
assert.ok(pinnedActionCount > 0, 'No GitHub Actions references were inspected');
...
const activeFiles = [
  ...
  ...(await walk('tests')),
  ...(await walk('docs')).filter((file) => !file.startsWith('docs/archive/')),
];
for (const file of activeFiles) {
  const source = await read(file);
  assert.doesNotMatch(source, /clerk/i, `Active Clerk reference remains in ${file}`);
}
