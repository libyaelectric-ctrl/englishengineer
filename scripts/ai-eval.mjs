...
async function runEvaluation() {
  ...
  for (const [index, testCase] of EVAL_CASES.entries()) {
    ...
    try {
      const response = await generateAIResponse(testCase.input, 'gpt-4.1-mini', 'openai');
      console.log(`Response length: ${response.split(' ').length} words.`);
      
      const missingKeywords = testCase.expectedKeywords.filter(kw => !response.toLowerCase().includes(kw));
