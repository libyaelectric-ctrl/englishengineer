...
const canonicalPath = path.join(root, 'data/canonical/grammar/grammar-rules.normalized.json');
const reportPath = path.join(root, 'data/canonical/grammar/grammar-validation-report.json');
...
for (const level of CEFR_LEVELS) {
  const seedPath = path.join(root, `src/data/grammar/by-level/${level.toLowerCase()}.seed.ts`);
  await fs.access(seedPath);
  if (!rules.some((rule) => rule.cefrLevel === level)) {
    throw new Error(`Grammar level ${level} is empty.`);
  }
}
console.log(`Grammar verification passed: ${rules.length} unique rules across A1-C2.`);
