...
const resolveVariableCount = (sourceFile, filePath, variableName) => {
      ...
      if (init && ts.isArrayLiteralExpression(init)) {
        ...
        for (const el of init.elements) {
          if (ts.isSpreadElement(el) && ts.isIdentifier(el.expression)) {
            count += resolveVariableCount(sourceFile, filePath, el.expression.text);
          } else {
            count++;
          }
        }
  ...
  if (importPath) {
    ...
    if (existsSync(resolvedFile)) {
      ...
      return resolveVariableCount(importedSourceFile, resolvedFile, variableName);
    }
  }
...
const content = {
  engineeringTemplates:
    countArrayElements('src/features/work-tools/work-tools.data.ts', 'BASE_ENGINEERING_TEMPLATES') +
    countArrayElements(
      'src/features/work-tools/work-tools.engineering.data.ts',
      'ENGINEERING_SEEDS'
    ),
  emailTemplates:
    countArrayElements('src/features/work-tools/work-tools.data.ts', 'BASE_EMAIL_TEMPLATES') +
    countArrayElements('src/features/work-tools/work-tools.email.data.ts', 'EMAIL_SEEDS'),
  phraseLibrary: getPhraseLibraryCount('src/features/work-tools/work-tools.phrase.data.ts'),
