...
const grammar = JSON.parse(
  await fs.readFile(path.join(root, 'data/canonical/grammar/grammar-rules.normalized.json'), 'utf8')
);
const vocabulary = JSON.parse(
  await fs.readFile(path.join(root, 'data/canonical/vocabulary/vocabulary.normalized.json'), 'utf8')
);
...
const grammarDomains = new Set(grammar.flatMap((rule) => rule.domainFit.map(normalize)));
const vocabularyKeys = new Set(
  vocabulary.flatMap((term) => [normalize(term.normalizedTerm), ...term.tags.map(normalize)])
);
...
const report = {
  ...
  structuralIntegrity: {
    ...
    grammarIdsUnique: new Set(grammar.map((rule) => rule.id)).size === grammar.length,
    vocabularyIdsUnique: new Set(vocabulary.map((term) => term.id)).size === vocabulary.length,
  },
  relationships: {
    grammarFitLinks,
  ...
  compatibility: {
    ...
    lifeWideVocabularyCount: vocabulary.filter((term) => term.isLifeWideEnglish).length,
