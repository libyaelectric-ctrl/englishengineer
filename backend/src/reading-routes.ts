...
const breaker = new CircuitBreaker('ReadingAI', 5, 30000);
...
const mapGenerated = (
  ...
  const text = typeof reading.passage === 'string' ? reading.passage : '';
  ...
  app.post(
    ...
    async (request: Request, response: Response, next: NextFunction) => {
      try {
        ...
        const category = discipline?.trim() || 'general';
        const resolvedLevel = level?.trim() || 'B2';
        const language = targetLanguage?.trim() || 'en';
        if (category !== 'general' && !READING_ITEMS.some((item) => item.category === category))
          throw new ApiError(
            400,
            'invalid_discipline',
            `Unsupported engineering discipline: ${category}`
          );
        const { value: item } = await getOrSet(
          `reading:ai:${category}:${resolvedLevel}:${language}`,
          TTL_SECONDS,
          async () => {
            const result = await breaker.execute(() =>
              aiService.complete('generateContent', {
                prompt: [
                  ...
                  'Focus skill: reading',
                  'Return ONLY valid JSON matching the content structure.',
                ].join('\n'),
                context: { discipline: category, targetLevel: resolvedLevel },
              })
            );
            const fallback =
              READING_ITEMS.find((candidate) => candidate.category === category) ??
              READING_ITEMS[0];
            if (result.mockMode || !result.structuredResult) return fallback;
