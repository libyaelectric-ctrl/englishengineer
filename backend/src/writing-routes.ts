...
const requiredScore = (value: unknown, name: string): number => { const score = typeof value === 'number' ? value : Number(value); if (!Number.isFinite(score)) throw new ApiError(502, 'invalid_writing_assessment', `AI assessment omitted ${name}.`); return Math.max(0, Math.min(100, Math.round(score))); };
const mapAiScores = (structured: Record<string, unknown>): Omit<WritingSubmissionRecord, 'id' | 'userId' | 'promptId' | 'content' | 'submittedAt' | 'status'> => {
  ...
  const weaknesses = Array.isArray(structured.weaknesses) ? structured.weaknesses.filter((value): value is string => typeof value === 'string') : [];
  return { score, grammarScore: requiredScore(raw.grammar, 'grammar'), vocabularyScore: requiredScore(raw.vocabulary, 'vocabulary'), coherenceScore: requiredScore(raw.clarity, 'clarity'), structureScore: Math.round((tone + score) / 2), feedback: weaknesses.length ? { grammar: weaknesses.slice(0, 2).join(' ') } : {} };
};
export const registerWritingRoutes = (app: Express, requireBackendAuth: RequestHandler, writingLimiter: RequestHandler, aiService: AiService): void => {
  app.get('/api/writing/prompts', requireBackendAuth, (request: Request, response: Response, next: NextFunction) => { try { userIdFrom(request); const limit = Math.min(Math.max(Number(request.query.limit) || 10, 1), 100); const offset = Math.max(Number(request.query.offset) || 0, 0); response.json({ items: WRITING_PROMPTS.slice(offset, offset + limit), total: WRITING_PROMPTS.length, limit, offset }); } catch (error) { next(error); } });
  app.post('/api/writing/submit', requireBackendAuth, writingLimiter, validateBody(WritingSubmitBodySchema), async (request: Request, response: Response, next: NextFunction) => {
    try {
      const userId = userIdFrom(request);
      checkCostLimits(userId);
      const { promptId, content = '' } = request.validatedBody as { promptId?: string; content?: string };
      const prompt = WRITING_PROMPTS.find((candidate) => candidate.id === promptId);
      let evaluation: ReturnType<typeof mapAiScores>;
      try {
        const result = await breaker.execute(() => aiService.complete('evaluateEngineeringEnglish', { prompt: `Evaluate this engineering student's written response.\n${prompt ? `Task: ${prompt.prompt}\n` : ''}Submission:\n${content}`, context: {} }));
        if (result.mockMode) throw new ApiError(503, 'writing_assessment_unavailable', 'Writing assessment requires a configured AI provider.');
        if (!result.structuredResult) throw new ApiError(502, 'invalid_writing_assessment', 'AI provider returned no structured assessment.');
        evaluation = mapAiScores(result.structuredResult);
      } catch (error) {
        if (error instanceof ApiError) throw error;
        throw new ApiError(503, 'writing_assessment_unavailable', 'Writing assessment is temporarily unavailable.');
      }
      const submission = await getLearningRepository().createWritingSubmission({ userId, promptId: promptId ?? 'unknown', content, ...evaluation, status: 'graded' });
