...
const handleDbError = (error: {
  ...
  value.status = error.status || 500;
  value.code = error.code || 'N/A';
  value.details = error.details || error.message || 'N/A';
  return value;
};
export const createSupabaseBillingRepository = (
  ...
  const wrappedFetch = async (url: string, init?: RequestInit) => {
    ...
    if (init?.headers) {
      ...
      if (headers instanceof Headers)
        headers.forEach((value, key) => {
          headersObj[key.toLowerCase() === 'authorization' ? 'Authorization' : key] = value;
        });
      else headersObj = { ...(headers as Record<string, string>) };
    }
    ...
    if (!response.ok) {
      ...
      logger.warn('Billing repo error body', { body: bodyText });
  ...
  return {
    ...
    async upsertSubscriptionStatus(userId, snapshot) {
      const { error } = await supabase
        .from('subscription_status')
        .upsert(mapSubscriptionSnapshot(userId, snapshot), { onConflict: 'user_id' });
      if (error) throw handleDbError(error);
    },
    async consumeTopupCredit(userId, requestId) {
      const { data, error } = await supabase.rpc('consume_ai_topup_credit', {
        p_user_id: userId,
        p_request_id: requestId,
      });
      if (error) throw handleDbError(error);
      const row = (Array.isArray(data) ? data[0] : data) as Record<string, unknown> | null;
      if (!row) throw new Error('Atomic top-up RPC returned no result.');
      return {
        consumed: row.consumed === true,
        duplicate: row.duplicate === true,
        remainingCredits: Number(row.remaining_credits) || 0,
      } satisfies TopupConsumptionResult;
    },
