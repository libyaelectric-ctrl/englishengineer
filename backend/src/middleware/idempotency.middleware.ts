...
interface PendingEntry {
  fingerprint: string;
  response: Promise<IdempotencyEntry | null>;
...
const scopeKey = (req: Request, key: string): string =>
...
const replay = (res: Response, entry: IdempotencyEntry): void => {
...
export const idempotencyKey = (options: IdempotencyOptions = {}) => {
  const {
    ...
    ttlMs = 86_400_000,
    store = globalIdempotencyStore || createMemoryStore(),
  } = options;
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const header = req.headers[headerName.toLowerCase()];
      if (header === undefined) return next();
      if (typeof header !== 'string' || header.length < 16 || header.length > 256)
        throw new ApiError(
          400,
          'invalid_idempotency_key',
          'Idempotency key must be a string between 16 and 256 characters.'
        );
      const scoped = scopeKey(req, header);
      const bodyFingerprint = fingerprint(req);
      const existing = await store.get(scoped);
      if (existing && Date.now() - existing.timestamp <= ttlMs) {
        if (existing.fingerprint && existing.fingerprint !== bodyFingerprint) throw conflict();
        replay(res, existing);
        return;
      }
      ...
      if (inFlight) {
        if (inFlight.fingerprint !== bodyFingerprint) throw conflict();
        const result = await inFlight.response;
        if (result) replay(res, result);
        else next();
        return;
      }
      let resolvePending!: (entry: IdempotencyEntry | null) => void;
      let rejectPending!: (error: unknown) => void;
      const response = new Promise<IdempotencyEntry | null>((resolve, reject) => {
      ...
      let settled = false;
      const settle = (entry: IdempotencyEntry | null): void => {
        if (settled) return;
        settled = true;
...
const createRedisStore = (
  ...
  const redisFetch = async (args: (string | number)[]): Promise<{ result: unknown }> => {
    ...
    try {
      ...
      if (!Object.prototype.hasOwnProperty.call(payload, 'result'))
        throw new Error('Redis response is missing result.');
  ...
  return {
    async get(key) {
      ...
      try {
        return JSON.parse(payload.result) as IdempotencyEntry;
      } catch (error) {
        throw idempotencyStoreUnavailable(error);
      }
    },
...
export const createIdempotencyStore = (
  ...
  config: {
    rateLimit?: { upstashUrl?: string; upstashToken?: string; storeTimeoutMs?: number };
  } = {},
  ...
  type === 'memory'
    ? createMemoryStore()
    : type === 'redis'
      ? createRedisStore(config, fetchImpl)
      : (() => {
