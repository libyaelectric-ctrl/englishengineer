...
const fetchWithTimeout = async (fetchImpl, url, init, timeoutMs) => {
  ...
  try {
    ...
    throw new ApiError(502, 'vocabulary_provider_unavailable', 'External vocabulary lookup is temporarily unavailable.');
  } finally {
...
const parseDictionaryResponse = (payload) => {
  ...
  return {
    ...
    phonetic: typeof entry.phonetic === 'string' ? entry.phonetic : (entry.phonetics?.find((item) => item?.text)?.text ?? null),
    definitions,
  };
};
...
const translateWithLibre = async (config, fetchImpl, text, targetLang) => {
  ...
  const response = await fetchWithTimeout(fetchImpl, config.libreTranslateUrl, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ q: text, source: 'en', target: targetLang, format: 'text', ...(config.libreTranslateApiKey ? { api_key: config.libreTranslateApiKey } : {}) }),
  }, config.timeoutMs);
  if (!response.ok) return null;
  const payload = await response.json();
  return typeof payload?.translatedText === 'string' ? { text: payload.translatedText, source: 'LibreTranslate' } : null;
};
...
export const createUpstashVocabularyCache = ({ url, token, timeoutMs = 3000, fetchImpl = fetch }) => ({
  async get(key) {
    ...
    try {
      const response = await fetchImpl(url, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(['GET', key]), signal: controller.signal });
      if (!response.ok) return null;
      const payload = await response.json();
  ...
  async set(key, value) {
    ...
    const timeoutId = setTimeout(() => controller.abort(), timeoutMs);
    try {
      await fetchImpl(url, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(['SET', key, JSON.stringify(value), 'EX', '604800']), signal: controller.signal });
    } catch (err) { console.warn('[vocabulary-cache-set]', err?.message); } finally { clearTimeout(timeoutId); }
  },
});
...
export const createVocabularyLookupService = (config, fetchImpl = fetch, cache = new Map()) => ({
  async lookup(query) {
    ...
    if (!response.ok) {
      throw new ApiError(response.status === 404 ? 404 : 502, response.status === 404 ? 'vocabulary_not_found' : 'vocabulary_provider_unavailable', response.status === 404 ? 'No external dictionary entry was found.' : 'External vocabulary lookup is temporarily unavailable.');
    }

    const parsed = parseDictionaryResponse(await response.json());
    let translation = null;
    try {
      translation = (await translateWithLibre(config, fetchImpl, parsed.definitions[0], targetLang)) ?? (await translateWithMyMemory(config, fetchImpl, parsed.definitions[0], targetLang));
    } catch { translation = null; }

    const result = { ...parsed, translation: translation?.text ?? null, source: 'Free Dictionary API', translationSource: translation?.source ?? null, cached: false };
